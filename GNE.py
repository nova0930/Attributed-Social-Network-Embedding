...
class GNE(BaseEstimator, TransformerMixin):
    def __init__(self, path, data, id_embedding_size, attr_embedding_size,
                ...
                epoch=100, random_seed = 2018, representation_size = 32):
        ...
        self.hidden_layer_size_1    = self.id_embedding_size + self.attr_embedding_size  # 256 dim
        self.hidden_layer_size_2    = (int) ((self.hidden_layer_size_1)/2) # 128
        self.hidden_layer_size_3    = (int) ((self.hidden_layer_size_2)/2) # 64
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default(), tf.device('/gpu:0'):
            ...
            self.id_embed           =  tf.nn.l2_normalize(tf.nn.embedding_lookup(self.weights['in_embeddings'], self.train_data_id), 1) # batch_size * id_dim
            ...
            self.attr_layer_1       =  tf.nn.tanh(tf.add(tf.matmul(self.attr_input_dropout, self.weights['attr_hidden_1']), self.weights['attr_bias_1'])) # batch_size * hidden_size_1
            self.dropout_attr_1     =  tf.nn.dropout(self.attr_layer_1, self.keep_prob)
            self.attr_layer_2       =  tf.nn.tanh(tf.add(tf.matmul(self.dropout_attr_1, self.weights['attr_hidden_2']), self.weights['attr_bias_2'])) # hidden_size_1 * hidden_size_2
            self.dropout_attr_2     =  tf.nn.dropout(self.attr_layer_2, self.keep_prob)
            self.attr_embed         =  tf.nn.tanh(tf.add(tf.matmul(self.dropout_attr_2, self.weights['attr_embeddings']), self.weights['attr_bias'])) # hidden_size_2 * attr_dim
            ...
            self.hidden_layer_1     = tf.nn.tanh(tf.add(tf.matmul(self.dropout_1, self.weights['hidden_layer_1']), self.weights['hidden_bias_1']))
            ...
            # This layer gives the joint representation for each node
            self.representation_layer  = tf.nn.tanh(tf.add(tf.matmul(self.hidden_layer_3, self.weights['representation_layer']), self.weights['representation_layer_bias']))
            ...
            # Optimizer. / learning rate for yeast = 0.005
    ...
    def _initialize_weights(self):
        ...
        all_weights['attr_embeddings']  = tf.Variable(tf.random_normal([self.hidden_size_2, self.attr_embedding_size]))  # attr_M * attr_dim
        ...
        all_weights['representation_layer']             = tf.Variable( tf.random_normal([self.hidden_layer_size_3, self.representation_size]))
